...
if __name__ == "__main__":
    ...
    batch_data = [
        ...
        "combined_stats_window_50_code_with_problems",
    ...
    CLAWS_OUTPUT_PATH = [
        ...
        "output/window_50/code_with_problems/output/",
    ...
    for threshold in THRESHOLDS:
        for idx, batch_id in enumerate(batch_data):
            SCORES_CSV_FILE = Path(CLAWS_OUTPUT_PATH[idx]) / f"scores/{batch_id}.csv"
        ...
        for idx, batch_id in enumerate(batch_data):
            ...
            print(f"\nBatch: {batch_id}")
            counts = compute_label_counts(scores, threshold=threshold, ontology="parser")
            plt.hist(counts, bins=range(min(counts), max(counts)+2), alpha=0.25, label=f"{batch_id}", edgecolor="black")
        plt.xlabel("Number of Recommended Labels per Exercise")
        plt.ylabel("Frequency")
        plt.title(f"Recommended Labels per Exercise (Threshold={threshold})")
        plt.legend()
        plt.grid(axis="y", linestyle="--", alpha=0.6)
        plt.tight_layout()
        plt.show()
